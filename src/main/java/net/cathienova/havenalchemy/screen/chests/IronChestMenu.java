...
import net.cathienova.havenalchemy.block.chests.IronChestBlockEntity;
...
public class IronChestMenu extends AbstractContainerMenu
{
    ...
    private static final int START_PLAYER_INV_Y = 112;
    private static final int START_PLAYER_HOTBAR_X = 8;
    private static final int START_PLAYER_HOTBAR_Y = 170;

    public IronChestMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(SLOTS));
    }

    public IronChestMenu(int pContainerId, Inventory playerInventory, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.IRON_CHEST_MENU.get(), pContainerId);
        checkContainerSize(playerInventory, 36);
        blockEntity = ((IronChestBlockEntity) entity);
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, ModBlocks.iron_chest.get());
    }
    ...
    public IronChestBlockEntity getBlockEntity() {
