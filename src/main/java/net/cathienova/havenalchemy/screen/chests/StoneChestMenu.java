...
import net.cathienova.havenalchemy.block.chests.StoneChestBlockEntity;
...
{
    public final StoneChestBlockEntity blockEntity;
    ...
    private static final int START_PLAYER_INV_Y = 75;
    private static final int START_PLAYER_HOTBAR_X = 8;
    private static final int START_PLAYER_HOTBAR_Y = 133;

    public StoneChestMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(SLOTS));
    }

    public StoneChestMenu(int pContainerId, Inventory playerInventory, BlockEntity entity, ContainerData data) {
        super(ModMenuTypes.STONE_CHEST_MENU.get(), pContainerId);
        checkContainerSize(playerInventory, 36);
        blockEntity = ((StoneChestBlockEntity) entity);
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, ModBlocks.stone_chest.get());
    }
    ...
    public StoneChestBlockEntity getBlockEntity() {
